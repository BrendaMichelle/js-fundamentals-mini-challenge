...
function printNameAndPhones(userArr) {
  for (i = 0; i < userArr.length; ++i) {
    console.log(userArr[i].name)
    console.log(userArr[i].phones.cell)
    console.log(userArr[i].phones.office)
  }
}
...
function takeATicketNumber(line) {
  ...
  const greeting = `Welcome. You are ticket number ${ticketNumber}`
  ++ticketNumber
  return greeting
}
...
function ticketNumberGeneratorFunc() {
  ...
  function takeATicketNumber(lineArr) {
    lineArr.push(newTicketNumber)
    const greeting = `Welcome. You are ticket number ${newTicketNumber}`
    ++newTicketNumber
    return greeting
  }
