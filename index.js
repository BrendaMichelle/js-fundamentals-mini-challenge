...
function drinkWater(currentThirstLevel) {
  const thirsty = 'Man I sure am thirsty';
  const hitTheSpot = 'Ahh that hits the spot';
  currentThirstLevel -= 1; 
  return [thirsty, hitTheSpot, currentThirstLevel];
} 
...
function printNameAndPhones(person) {
  person.forEach((person) => {
    const name = person.name;
    const cell = person.phones.cell;
    const office = person.phones.office;
    console.log(name);
  ...
  //   return user.forEach(obj);
  //  }
    // return name.phones

console.log(printNameAndPhones(users))
// => "Duane"
// => "Cell: 555-123-4567"
// => "Office: 555-456-7890"
// => "Liza"
