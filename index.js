...
function sameSameButDifferent(num, maybeNum){
  if (typeof maybeNum === 'number' && num === maybeNum){
    return "same same"
...
function takeATicketNumber(line){
  let nextNum = extraLine.length+1
  line.push(nextNum)
  extraLine.push(nextNum)
  return `Welcome, You are ticket number ${nextNum}`
}
...
function ticketNumberGeneratorFunc(){
  ...
  function takeATicketNumber2(line){
    let nextNum = extraLine.length+1
    line.push(nextNum)
    extraLine.push(nextNum)
    return `Welcome, You are ticket number ${nextNum}`
  ...
  return takeATicketNumber2
}

const newLine = [] // goals is NOT to use a global variable to store data
