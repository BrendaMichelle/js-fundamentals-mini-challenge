...
function updateGrade(student, number) {
...
function ticketNumberGeneratorFunc() {
  let counter = [];
  function newTakeATicketNumber(arr) {
    if (!arr.length) {
      arr.push(1);
      counter.push(1);
    } else {
      arr.push(parseInt(`${arr.length + 1}`));
      counter.push(parseInt(`${arr.length + 1}`));
    }
    return `Welcome. You are ticket number ${counter.length}`;
  }
  return newTakeATicketNumber
}
const newLine = []
