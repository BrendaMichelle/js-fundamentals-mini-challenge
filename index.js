...
const name = 'Henry Velasquez'
console.log(name)   // WHY DOES NAME HAVE A STRIKETHROUGH?
// => "Raffy"
...
function sameSameButDifferent(num, maybeNum) {
  if (num === maybeNum) {
    ...
    console.log( 'same same but different' ) //DO I PUT SPACES OR NO?
  } else {
...
function updateGrade(student, grade) {
  student['grade'] = grade   // WHAT EXACTLY IS THE CORRECT JSON OBJECT NOTATION. I TRIED [grade:] and [:grade] and they did not work
}
...
const x = function takeATicketNumber(line) {
  ticket += 1
  line.push(ticket)
  return `Welcome. You are ticket number ${ticket}`
}
