...
function printNameAndPhones(users){
  users.forEach(user => {
    ...
    for (const number in user.phones){
      let properName = number.charAt(0).toUpperCase() + number.slice(1)
      console.log(`${properName}: ${user.phones[number]}`)
    }
  });  
  }
...
function ticketNumberGeneratorFunc() {
  ...
  return function takeATicketNumber2 (newLine){
    console.log(`Welcome. You are ticket number ${newTicketNumber}`)
