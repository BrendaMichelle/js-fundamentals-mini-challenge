...
function sameSameButDifferent (num, maybeNum) {
  if (!Number.isNaN(num) && Number.isFinite(maybeNum)) {
    if (num === maybeNum) {
      return "same same"
    }
    else {
...
function updateGrade (studentObject, gradeNum) {
  return studentObject["grade"] = gradeNum
}
...
function ticketNumberGeneratorFunc () {
  return function takeATicketNumberFunc () {
