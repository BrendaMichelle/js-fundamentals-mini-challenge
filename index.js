...
//console.log(name) name can't be accessed before initialization!
//=> "Raffy"
let name = "Not Raffy" 
console.log(name);
//=> TypeError
...
function printNameAndPhones(usersArr) {
...
let lastserverd = null

function takeATicketNumber(line) {
  if (line.length == 0 && lastserverd == null) {
    line.push(1);
    lastserverd = 1;
  } 
  else {
    line.push(lastserverd + 1);
    lastserverd++;
  }

  return `Welcome. You are ticket number ${lastserverd}`;
...
function ticketNumberGeneratorFunc(newline){
  newLine.push(1);
  return `Welcome. You are ticket number ${newLine[newLine.length] - 1 }`

}

console.log("Im not sure how to do the last one without googling 😢")
